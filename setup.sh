...
# Install spaCy models for NER (optional - app works with regex-only if not available)
echo "Installing spaCy models..."
python -m spacy download en_core_web_sm 2>/dev/null || echo "Warning: en_core_web_sm not available - using regex-only detection"
python -m spacy download en_core_web_lg 2>/dev/null || echo "Warning: en_core_web_lg not available - using regex-only detection"
